...
* [`src/cljs/coin_watch/`](src/cljs/coin_watch/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/coin_watch/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `coin-watch` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/coin_watch/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns coin-watch.example
  (:require [coin-watch.config :as config])
